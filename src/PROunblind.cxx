...
namespace PROfit{
    ...
    int PROunblind_Stage1( const PROconfig &config, const PROpeller &prop, PROmetric *metric , PROseed &myseed, PROdata data, size_t nthread, std::string final_output_tag){
        ...
        //Did we find a massivly better global fit down the line, if so we ill repeat...
        Eigen::VectorXf loopseed;
        ...
        while(true){
            ...
            float chi2;
            if(loopseed.size()>0){
                 chi2 = fitter.Fit(*metric,loopseed); 
            }else{
                 chi2 = fitter.Fit(*metric); 
            }
