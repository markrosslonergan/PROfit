            ...
            if(chi2_syst < chi2_osc && !args.gof_mode) {
                ...
                if(chi2_osc_retry < chi2_osc) {
                    log<LOG_INFO>(L"%1% || Refit improved chi2_osc from %2% to %3%") 
                        % __func__ % chi2_osc % chi2_osc_retry;
                    chi2_osc = chi2_osc_retry;
                    fitter_osc.best_fit = fitter_osc_retry.best_fit;
                }
                if(chi2_syst < chi2_osc) {
                    log<LOG_WARNING>(L"%1% || Delta chi2 still negative after refit. Attempting enhanced global search") % __func__;
