...
public:
    ...
    PROdata(const PROconfig & config, const PROspec &in_prospec ){
        Eigen::VectorXf data_vec = CollapseMatrix(config, in_prospec.Spec());
        Eigen::VectorXf err_vec_sq = in_prospec.Error().array().square();
